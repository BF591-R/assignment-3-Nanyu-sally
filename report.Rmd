...
```{r affy_to_hgnc}
# 1. 随便选 3 个探针 ID
test_ids <- c("202860_at", "1553551_s_at", "204340_at")
...
# 3. 只查这 3 个，看它几秒钟能不能出结果
test_mapping <- biomaRt::getBM(
...
ggplot(long_format_data, aes(x = hgnc, y = values, fill = gene_set)) +
  geom_boxplot() +
  scale_fill_manual(values = c("good" = "#4DAF4A", "bad" = "#E41A1C")) +
  labs(
    title = "Gene Expression Levels of Interesting Gene Sets",
    subtitle = "Comparing Good vs. Bad Category Genes",
    x = "HGNC Gene Symbol",
    y = "Expression Value (log2 Intensity)",
    fill = "Gene Category"
  ) +
  theme_minimal() +
